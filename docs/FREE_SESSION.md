...
列表每行「更多」中的“从列表移除”先确认，成功后移除该行，失败保留该行及原入口并允许重试；桌面和手机共用此行为。
...
同名显示类型、范围/组及版本，方向键和 Enter 或点击选中。精确解析尚未返回时禁发；选择失败释放等待，迟到成功不清空后来输入，离开会话后不写回旧选择。补全选中后把 `@查询词` 替换为完整名称与一个空格，保留原句前文；精确引用附在消息上。引用达到上限或解析失败时保留原输入，移除标记不删除名称。补全与信息区“引用到对话”形成同一种可删除、可打开的标记。
只打开详情、来源、候选或 explorer 不写入消息 refs。历史消息标出已固定显式引用；浏览也不改变 task 的目标。
整稿／选区反馈复用同一个 composer 和现有 `message`／`refs` 请求：显式点击反馈入口后固定正在阅读的精确引用，选区作为可编辑引文追加到原消息。
选区只接受当前已加载内容中的同一 Markdown 正文；切换版本、重新读取或原文不可用后不能复用旧选区。读取和选择本身不发送消息。
引用达到 8 项且当前稿尚未引用时保留原消息和全部引用并提示上限；已引用同一版本时去重。移除引用只移除结构化 refs，已写入消息的引文仍可编辑。
...
信息区正文优先，标题、版本选择、保存状态和操作常驻；所属范围、新建/更新、冻结基础版本、来源、完整属性与技术标识集中在默认收起的「稿件详情」（正式资料为「资料详情」），切换对象重新收起。初始化资料仅展示一份正文，保存范围移入详情；反向母版的保留、改写与排除说明也在同一入口。历史资产/删除提示与保存冲突、待核实状态仍直接显示。
正式保存只显示真实 receipt；模型回复不产生保存标识。
...
桌面双栏默认讨论 40%、稿件 60%，可通过分隔条拖动或键盘调节并在当前标签页恢复。全局导航可折叠，窄屏保留导航访问和原视图切换。
这些浏览器偏好不属于创作输入，不改变任务、候选、引用或保存目标。

界面中的「草稿／版本」选择仍绑定 group_id 与 draft_id，不改变后端候选身份。信息区以独立状态行呈现当前第 N 稿和中文保存状态，版本选择与并排对照集中排列。完整引用、原始目标和操作 ID 收在统一的稿件详情；短标识仅在发现结果、无法取得可读名称或同名引用消歧时保留。引用标记默认使用名称、类型及版本，完整身份仍保留在 title 和详情。没有 receipt 时不能从模型文字、claim 或折叠状态推导保存成功。

## 模式导航与资产往返（MWT-030）

完整候选的「准备保存此稿」只准备现有 composer 消息和精确 ref，不调用保存 API。用户发送后仍由原意图解释和授权流程核验。
准备动作保留已有文字与资产引用；已有其他候选引用时提示先移除，避免原样保存的唯一候选边界含糊。引用超过 8 项时整体不变。
已保存或成员引用不提供整稿保存准备入口；存在保存操作记录时，准备按钮保持禁用并沿用原核实或冲突提示，结果仍以真实收据为准。

同组候选支持独立的版本对照对话框，左右通过现有精确读取接口分别获取所选版本，展示正文与包内资料。
选择对照版本不修改主阅读对象、composer refs 或任务目标；读取失败在对应侧显示，不以另一侧或当前资产替代。
关闭对照保留原输入、阅读位置和主版本，并回到触发按钮。此视图只供阅读，不提供手工改稿或自动差异合并。

默认无 hash 和 `#free/new` 打开自由创作。主导航为「新建创作、最近会话、故事书架、角色库、世界观」，桌面和手机使用同一导航内容。`#free/conversations` 按 conversation 展示可读的最近任务时间、最近消息摘要、中文状态、草稿组数、已确认保存数和正式关联名称；无正式成果的会话同样可恢复，多资产关联不重复会话行。列表读取现有 conversations/tasks/groups 分页；正式关联名称按目标去重后经既有 library/stories 点读补充，读取失败只降级该名称，保留链接，完整 ID 可从每行「更多」查看。初始资料通过所属会话的 scoped references API 按精确版本补充名称，读取失败或原版不可取得时显示原版本不可用，不以当前名称替换历史内容。标题最多两行、最近消息一行，时间和计数集中在底栏，移除入口收在「更多」。committed 缺少收据时显示保存结果待核实，模型输出不作为保存证据。不新增创意对象或业务保存目标。
...
`#creative/new`、`#creative/conversations`、`#creative/conversation/:id`、`#creative/draft/:storyId/:draftId`、`#story/:id/creative` 与 `#story/:id/draft/:draftId` 继续使用原协议；“更多”集中提供“旧创作会话与草稿”和“旧版新建故事”，故事页面的“更多”另提供“旧故事创作会话”。历史不迁移、不删除，旧侧栏仍限原资产／阅读页面，自由会话不挂载 WritingHost。关联会话默认折叠，故事阅读页的“带此故事新建会话”进入 `#free/new/story/:id`，原关联会话从折叠区恢复。
...
信息区展示各成员完整正文；新增/更新、冻结的基础业务版本、完整属性、母版或本会话候选的精确入包来源集中到「稿件详情」，来源仍可打开。参考来源独立列示，引用不自动增加成员。浏览、切换旧稿和新稿提醒不授权保存。资料真实收据列出各成员模式与结果版本，可进入对应资料阅读页并返回原会话。旧十工具/world-v1 会话提示资料能力限制并提供主动新建入口，原聊天、候选和授权不转移。
