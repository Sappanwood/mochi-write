...
自由会话的双关注点交互可通过 [隔离合成原型](prototypes/free-session/README.md) 实际操作和重演。
原型区分当前阅读、显式引用与每轮保存目标，覆盖成果组、旧版本、目标待解析和手机往返；
原型只保存 MWT-024 的隔离设计验证与历史走查；正式 `#free` 界面已消费该设计并接入真实业务 API。
本文 v1 生命周期/故事会话仍保留查看即引用行为与固定工具快照，新 v2 只由显式引用操作附加上下文。


## 自由会话后端 v2（MWT-025）

新增本人 API、独立会话身份、目标后绑定与跨分区恢复见 [自由会话服务](FREE_SESSION.md)。
旧本文 v1 路由与 UI 继续使用；自由会话候选组与角色 draft、资产/候选发现及显式引用 API 已接通；角色母版按操作绑定的精确保存、版本冲突与原 OP 收据恢复已接通；故事初始化/首章/续章与反向独立角色母版已在原 session 接通，沿用零章限制、完整独立快照和各轮独立 OP；`#free/new` 与 `#free/conversation/:id` 已接入双关注点工作区，详见自由会话服务的浏览器契约；默认首页与主导航已接入自由会话。

自由会话新增单故事资料修订通过独立工具和成员收据保存，旧初始化/首章与侧栏行为保留。资料版本、事务及恢复契约见 [自由会话](FREE_SESSION.md#单故事资料正式保存)。
