...
旧本文 v1 路由与 UI 继续使用；自由会话候选组与角色 draft、资产/候选发现及显式引用 API 已接通；角色母版按操作绑定的精确保存、版本冲突与原 OP 收据恢复已接通；故事初始化/首章/续章与反向独立角色母版已在原 session 接通，沿用零章限制、完整独立快照和各轮独立 OP；`#free/new` 与 `#free/conversation/:id` 已接入双关注点工作区，详见自由会话服务的浏览器契约；主导航另行接入。
