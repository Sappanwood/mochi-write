...
`/api/creative/free` 与 v1 路由并存；记录使用 free 命名空间，callback 按 protocol_version 分流，不放宽旧 story scope。十工具快照、独立目标核验、任务预算、本人 API、候选组/不可变版本、统一显式引用解析、有界发现、来源与 OP 恢复详见 [自由会话服务](FREE_SESSION.md)。
