...
角色库卡片与角色详情已完成 UI 调整：头像和身份信息集中在页首，正文限宽阅读，头像与提示词按需展开，关联会话与删除操作收纳到「更多」。[发布与自动检查](https://github.com/Sappanwood/mochi-write/actions/runs/36125658608)通过；线上合成角色已验证桌面与手机布局、头像上传保存及刷新读取、关联会话入口和删除，测试角色已清理。
书架与阅读页已完成 UI 调整：统一封面卡片，手机使用单列横向布局；阅读工具区更紧凑，选章后收起目录并将焦点带回正文，窄屏助手入口不再遮挡卡片和翻章按钮。[发布与自动检查](https://github.com/Sappanwood/mochi-write/actions/runs/36127776564)通过；线上已只读验证书架布局、合成故事阅读、目录选章和手机助手位置，长章节前后翻页由本地 E2E 覆盖。
...
头像支持文件选择或点击粘贴图片入口后按 Ctrl/⌘+V，粘贴后先裁剪、确认上传。
头像、提示词与角色版本一起保存；上传后须点击「保存头像与提示词」才关联角色，冲突可保留草稿后对照最新版本。
「整理 2.5D 提示词」只准备已有文字会话的输入，用户发送后调用现有模型；输出由用户复制回提示词栏并保存，不直接调用生图模型。默认采用 3:4 构图，加入符合角色设定的生活、学习或工作场景、自然动作与光线。
原图在浏览器裁剪为 3:4 竖幅，服务器重新编码为 768×1024 JPEG；原图不保存。旧版 512×512 头像继续支持读取及导入导出。旧头像与未采纳上传保留，不自动删除，以保护历史版本及故事快照。
